...
- Public types exported from the root: `Page<T>`, `Single<T>`, `EnvelopeFamily`, `Hip4Envelope`, `Address`, `Coin`, `Hex`, `Side`, `Wei`, plus per-resource params and row types (see [`packages/sdk/src/index.ts`](./packages/sdk/src/index.ts)).
...
```
hypedexer-sdk/
├── packages/sdk/          @hypedexer/sdk — the published package
│   ├── src/               source code (16 resources, transport, errors, types)
│   ├── test/              501 unit tests + type regression
│   ├── CHANGELOG.md
│   └── README.md          copy of this readme (shipped on npm)
├── examples/              six runnable scripts (fills, ws, errors, …)
├── PLAN.md                design doc — envelope strategy, quirks audit
├── ENDPOINTS.md           endpoint inventory
├── TYPES.md               type-shape reference
└── .github/workflows/     ci.yml, release.yml (tag-triggered npm publish)
```
...
pnpm --filter @hypedexer/sdk test
pnpm --filter @hypedexer/sdk build
